...
function initWeb(){
  ...
  if (user) {
    ...
    //get currId แทน start
    //เรียก start จาก function getCurrid
...
function GeneralEducation(currNameId){
  ...
  var query = firebase.database().ref("CPE/"+currNameId+"/Course/").orderByChild("Ccode");
...
function CdeleteOnClick(trKey,currNameId){
  ...
  $(document).ready(function(){
    ...
    var firebaseRef = firebase.database().ref("CPE/"+currNameId+"/Course/"+trKey);
...
function MajorEducation(currNameId){
  ...
  var fragment = document.createDocumentFragment();
  var table = document.createElement("table");
  var query = firebase.database().ref("CPE/"+currNameId+"/MajorCourse/").orderByChild("Ccode");
...
function MdeleteOnClick(trKey, currNameId){
  ...
  $(document).ready(function(){
    ...
    var firebaseRef = firebase.database().ref("CPE/"+currNameId+"/MajorCourse/"+trKey);
...
function addCourse(){
  ...
  if(intCheck == 1){
    ...
    var firebaseRef=firebase.database().ref("CPE/"+currNameIdGlobal+"/Course/");
...
function addMCourse(){
  ...
  if(intCheck == 1){
    ...
    var firebaseRef=firebase.database().ref("CPE/"+currNameIdGlobal+"/MajorCourse/");
...
function addNewCurriculum(){
  ...
  if(!AddCurrNameId){
    intCheck = 2;
    alert("กรุณากรอก Curriculum CPE year");
  }
  else if(!AddCurrName){
    intCheck = 2;
  ...
  if(intCheck == 1){
    ...
    var firebaseRef=firebase.database().ref("CPE/"+AddCurrNameId+"/Info/");
...
function getCurrId(){
  ...
  var query = firebase.database().ref("CPE");
...
function getCurrName(currNameId,i){
  ...
  var query = firebase.database().ref("CPE/"+currNameId+"/Info/");
