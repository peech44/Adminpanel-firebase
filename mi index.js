...
function GeneralEducation(){
  ...
  var query = firebase.database().ref("MI/Course/").orderByChild("Ccode");
...
function CdeleteOnClick(trKey){
  ...
  var firebaseRef = firebase.database().ref("MI/Course/"+trKey);
...
var fragment = document.createDocumentFragment();
  var table = document.createElement("table");
  var query = firebase.database().ref("MI/MajorCourse/").orderByChild("Ccode");
  $(document).ready(function(){
  query.once("value").then(function(snapshot){
    var i = 1;  
    snapshot.forEach(function(childSnapshot){
...
function MdeleteOnClick(trKey){
  ...
  var firebaseRef = firebase.database().ref("MI/MajorCourse/"+trKey);
  firebaseRef.remove().then(function(){
    console.log("Remove succeded");
    //re display #MCcdoe
    //if not the table of #Ccdoe will be duplicate (table1+table2)
...
function addCourse(){
  ...
  if(intCheck == 1){
    ...
    var firebaseRef=firebase.database().ref("MI/Course/");
...
function addMCourse(){
  ...
  if(intCheck == 1){
    ...
    var firebaseRef=firebase.database().ref("MI/MajorCourse/");
